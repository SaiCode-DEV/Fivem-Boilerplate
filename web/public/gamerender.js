const config = {
  ...
  defaultBlurStrength: 5, //  blur strength if not provided in the data attribute
  zindex: '-1', // z-index of game render
  renderColour: [0, 0, 0, 0], // rgba (for debugging cuttouts)
...
function createGameView (canvas, resolutionScale = config.resolutionScale) {
  ...
  function updateGlassElements () {
    ...
    for (const element of glassElements) {
      ...
      if (!Object.hasOwn(element.dataset, 'blurStrength')) {
        element.dataset.blurStrength = config.defaultBlurStrength.toString();
      }
    }
  }
  ...
  function debouncedUpdate () {
    ...
    updateTimeout = setTimeout(updateGlassElements, 100);
  }

  // Listen for visibility changes
  window.addEventListener('message', event => {
    if (event.data?.action === 'uiChanged') {
      debouncedUpdate();
    }
  });
  ...
  function render () {
    ...
    for (const element of glassElements) {
      ...
      const borderRadius = Number.parseFloat(getComputedStyle(element).borderRadius);
      const isCircle = getComputedStyle(element).borderRadius.includes('%')
        && Number.parseFloat(getComputedStyle(element).borderRadius) >= 50;
      const blurStrength = Number.parseFloat(element.dataset.blurStrength) || config.defaultBlurStrength;
  ...
  const gameView = {
    ...
    updateElements: updateGlassElements,
    start: () => {
      gl.clearColor(...config.renderColour);
      updateGlassElements(); // Initial query
      render();
    }
  };
