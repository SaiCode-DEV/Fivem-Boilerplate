...
(async () => {
  try {
    ...
    console.error('[StockMarket] Failed to run migrations. Server may not function correctly.', error);
  }
})();
...
onNet(`${cache.resource}:getCounter`, async () => {
  const src = source;
  const result = await exports.oxmysql.query_async('SELECT counter_value FROM stockmarket_counter LIMIT 1');
...
onNet(`${cache.resource}:incrementCounter`, async () => {
  await exports.oxmysql.execute_async('UPDATE stockmarket_counter SET counter_value = counter_value + 1 WHERE id = 1');
